	...
	const getResult = (q: string): Array<Group> => {
		...
		if (skills.length) {
			groups.push({
				...
				//name: "Skills",
				name: get(_)('nav.skills.title'),
				items: skills.map((it) => ({
					name: it.name,
					logo: $mode === 'dark' ? it.logo.dark : it.logo.light,
					//link: `/skills/${it.slug}`,
					link: get(_)('nav.skills.href')+`/${it.slug}`,
					color: it.color
				}))
			});
		}
		...
		if (projects.length) {
			groups.push({
				icon: 'i-carbon-cube',
				//name: "Projects",
				name: get(_)('nav.projects.title'),
				items: projects.map((it) => ({
					name: it.name,
					logo: $mode === 'dark' ? it.logo.dark : it.logo.light,
					//link: `/projects/${it.slug}`,
					link: get(_)('nav.projects.href')+`/${it.slug}`,
					color: it.color
				}))
			});
		}
		...
		if (experience.length) {
			groups.push({
				...
				//name: "Experience",
				name: get(_)('nav.experience.title'),
				items: experience.map((it) => ({
					name: it.name,
					logo: $mode === 'dark' ? it.logo.dark : it.logo.light,
					//link: `/experience/${it.slug}`,
					link: get(_)('nav.experience.href')+`/${it.slug}`,
					color: it.color
				}))
			});
		}
		...
		if (education.length) {
			groups.push({
				...
				//name: "Education",
				name: get(_)('nav.education.title'),
				items: education.map((it) => ({
					name: it.degree,
					logo: $mode === 'dark' ? it.logo.dark : it.logo.light,
					//link: `/education/${it.slug}`,
					link: get(_)('nav.education.href')+`/${it.slug}`,
...
<SearchPage title={$_('nav.search.title')} {onSearch}>
	{#if result.length === 0}
		<EmptyResult />
	{:else}
							...
							<FancyCard color={item.color} href={href(item.link)}>
								<CardContent class="flex flex-row items-center gap-4">
									...
									<Tooltip openDelay={300}>
										<TooltipTrigger>
											<CardTitle class="line-clamp-2 truncate text-ellipsis text-left">{item.name}</CardTitle>
										</TooltipTrigger>
