...
<FancyCard class="flex h-full flex-col" color={project.color} href={href(`/projects/${project.slug}`)}>
	<CardHeader class="flex w-full flex-col gap-4">
		...
		<div class="flex w-full flex-row items-center gap-1 overflow-x-hidden">
			<CardTitle class="h-auto min-w-0 flex-1 overflow-x-hidden">
				<Tooltip>
					<TooltipTrigger class="w-full overflow-y-auto overflow-x-hidden truncate text-ellipsis text-nowrap text-left">
						{project.name}
					</TooltipTrigger>
	...
	<CardContent class="flex flex-1 flex-col gap-4">
		...
		<Muted className="flex-1">{ellipsify(project.abstract, 320)}</Muted>
